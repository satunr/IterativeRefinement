...
### Func.py.
<p align="justify">
This set of functions defines a workflow for building and simulating diffusion on a directed disease or gene network, where edges are weighted to represent transmission or regulatory influence. It includes utilities to construct GENIE-style graphs from matrices, simulate cascade-based spread processes (both chain-tracking and aggregate versions), perform permutation testing for statistical comparison, and iteratively modify network structure by removing or updating edges based on diffusion dynamics.
</p>
...
### Case_study.py.
<p align="justify">
The case study simulates patient-level disease trajectories by generating weighted random walks on the GENIE-derived disease graph and then clusters these sequences using agglomerative clustering with a precomputed pairwise distance structure. It evaluates the optimal number of trajectory clusters via internal validation scores (e.g., silhouette/Calinski–Harabasz) and analyzes temporal edge frequencies within each cluster to identify dominant disease transition patterns.
</p>
